...
vec3 Triangle::math_normal() const { return ((p1 - p0).cross(p2 - p0)).unit(); }
vec3 Triangle::normal() const { return n; }
void Triangle::auto_normal() { n = math_normal(); }
std::pair<double, vec3> Triangle::intersect(vec3 pos, vec3 dir) const
{
    const int eps = 1e-6;
    vec3 tpos = p0;
    vec3 tdir = n;
